      ...
      <CardContent className="w-full border-4 border-chestnut h-[auto]">
        ...
        <form onSubmit={handleEditDetails} className="w-full h-full flex flex-col gap-2">
          <Box className="w-full border h-full flex flex-col gap-2 p-2">
            ...
            {JSON.parse(localStorage.getItem("data"))?.role !== "kingofjungle" && <EditField icon={<TbShieldStar size={25} />} value={role} onChange={handleRoleChange} isSelect options={[{ value: "", label: "Select Role", disabled: true }, { value: "kingofjungle", label: "KING OF JUNGLE" }, { value: "queenofjungle", label: "QUEEN OF JUNGLE" }]} />}
